...
describe('bns route', () => {
  ...
  describe('GET', () => {
    ...
    it('should return 404 when address is not eligible for bns discount', async () => {
      mockGetWalletProofs.mockRejectedValue(
        new MockProofsException(
          'address is not eligible for [bns_discount] this discount.',
          404,
        ),
      );
      ...
      expect(data).toEqual({
        error: 'address is not eligible for [bns_discount] this discount.',
      });
    });
    ...
    it('should call getWalletProofs with BNSDiscount namespace', async () => {
      const mockResponse: SuccessResponse = {
        address: validAddress,
        namespace: 'bns_discount',
        proofs: ['0xproof'],
