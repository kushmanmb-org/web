...
describe('cbid route', () => {
  ...
  describe('GET', () => {
    ...
    it('should return 404 when address is not eligible for cbid discount', async () => {
      mockGetWalletProofs.mockRejectedValue(
        new MockProofsException(
          'address is not eligible for [basenames_cbid_discount] this discount.',
          404,
        ),
      );
      ...
      expect(data).toEqual({
        error: 'address is not eligible for [basenames_cbid_discount] this discount.',
      });
    });
    ...
    it('should convert address to lowercase before calling getWalletProofs', async () => {
      const upperCaseAddress = '0xABCDEF1234567890123456789012345678901234';
      const mockResponse: SuccessResponse = {
        address: upperCaseAddress.toLowerCase(),
        namespace: 'basenames_cbid_discount',
        proofs: ['0xproof'],
        discountValidatorAddress: '0x502df754f25f492cad45ed85a4de0ee7540717e7',
      };
    ...
    it('should call getWalletProofs with caseInsensitive set to false', async () => {
      const mockResponse: SuccessResponse = {
        address: validAddress.toLowerCase(),
        namespace: 'basenames_cbid_discount',
        proofs: ['0xproof'],
      ...
      // Verify that the 4th argument (caseInsensitive) is false
      expect(mockGetWalletProofs).toHaveBeenCalledWith(
        expect.any(String),
        expect.any(Number),
        expect.any(String),
